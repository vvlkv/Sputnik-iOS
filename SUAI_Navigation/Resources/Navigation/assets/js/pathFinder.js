...
function RoomFinder(isTwoPointsSearch)
{
	...
	if (isTwoPointsSearch)
	{
		...
		for (var i=0;i<allParts.length;++i)
		{
			for (var j=0;j<allParts[i].length;++j)
			{
				if ( (curInd = allParts[i][j].name.indexOf(srcRoom)) > -1)
				{
					...
					var tmpPos = -1;
					var element;
					if ((tmpPos=allParts[i][j].name.indexOf(" "))>0)
					{
						var tmpPos2;
							if ((tmpPos2=allParts[i][j].name.indexOf(";"))>0)
							{								
								var tmpPos2 = allParts[i][j].name.lastIndexOf(";",curInd);
								var tmpPos3 = allParts[i][j].name.indexOf(";",curInd);
								tmpPos2 = (tmpPos2>-1)?tmpPos2+2:0;
								tmpPos3 = (tmpPos3>-1)?tmpPos3:allParts[i][j].name.length;
								
								tmpPos = allParts[i][j].name.indexOf(" ",tmpPos2);
								tmpPos = (tmpPos>-1)?tmpPos:allParts[i][j].name.length;
								if (tmpPos > tmpPos3)
									tmpPos = tmpPos3;
								var tmpstr = allParts[i][j].name.substr(tmpPos2,tmpPos-tmpPos2);
								element = SVGRoot.getElementById("aud" + tmpstr);
							}
							else
							{
								var tmpstr = allParts[i][j].name.substr(0,tmpPos);							
								element = SVGRoot.getElementById("aud" + tmpstr);							
							}
						
					}
					else
						element = SVGRoot.getElementById("aud" + srcRoom);
				...
				if ((curInd = allParts[i][j].name.indexOf(dstRoom)) > -1)
				{
					...
					if ((tmpPos=allParts[i][j].name.indexOf(" "))>0)
					{
						...
						element = SVGRoot.getElementById("aud" + dstRoom);
		...
		if (lastStartAudit == null)
		{
	//		ShowCustomAlert("Начальная аудитория не добавлена в базу!");
			return -1;
		}
		else
		{
			lastStartAudit.style.opacity = 1;
			NavigateToElementOnScreen(lastStartAudit);
		}
		
		if (lastEndAudit == null)
		{
	//		ShowCustomAlert("Конечная аудитория не добавлена в базу!");
			return -2;
		}
		else
			lastEndAudit.style.opacity = 1;
	}
	else
	{
		if (dstRoom=="")
